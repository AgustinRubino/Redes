...
namespace Redes
{
    ...
    public class PlayerController
    {
        ...
        Player _player;
        InputHandler _input;
        public void Set(Player player, InputHandler inputHandler)
        {
            _player = player;
            _input = inputHandler;
        }
        ...
        private void HandleJump()
        {
            if (_input.JumpInput)
            {
                ...
                _player.RB.AddForce(Vector3.up * _jumpForce, ForceMode.Impulse);
                OnJumped?.Invoke();
            }
        }
        ...
        private void HandleDash()
        {
            if (_input.DashInput)
            {
                ...
                {
                    DashTimer = 0;
                    _player.RB.AddForce(CurrentVelocity.normalized * _dashForce, ForceMode.Impulse);
                    OnDashed?.Invoke();
                }
            }
        ...
        private void HandleSteering()
        {
            ...
            _player.RB.MoveRotation(_player.RB.rotation * rot);


            var result = _player.transform.forward * Speed * MoveDirection;
            result = result - CurrentVelocity;

            _player.RB.AddForce(result * 1 / Time.fixedDeltaTime, ForceMode.Acceleration);
        }
        ...
        private void Breaking()
        {
            ...
            _player.RB.AddForce(-_player.transform.forward * _breakForce * -_input.Vertical, ForceMode.Acceleration);
        }

        private void Accelerate()
        {
            if (Speed > maxSpeed) return;
            _player.RB.AddForce(_player.transform.forward * _acceleration * _input.Vertical, ForceMode.Acceleration);
        }
        private void Deaccelerate()
        {
            ...
            _player.RB.AddForce(-CurrentVelocity.normalized * _deacceleration, ForceMode.Acceleration);
        }

        public void GetHit(Vector3 position, Vector3 force)
        {
            _player.RB.AddForceAtPosition(force, position, ForceMode.Impulse);
        }
    }
}